...
class ModelTraining:
    ...
    def train(self, optimizer = Optimizer) -> None:
        ...
        logging.info("the train method of Model trainer class")
        try:
            self.model.train()
    ...
    def initiate_model_trainer(self) -> ModelTrainingArtifact:
        try:
            ...
            optimizer: Optimizer = torch.optim.SGD(
                model.parameters(), **self.model_training_config.optimizer_params
            )

            scheduler: _LRScheduler = StepLR(
                optimizer=optimizer, **self.model_training_config.scheduler_params
            )
            ...
            bentoml.pytorch.save_model(
                ...
                model=model,
                custom_objects={
                    self.model_training_config.train_transforms_key: train_transforms_obj
                },
            )

            model_trainer_artifact: ModelTrainingArtifact = ModelTrainingArtifact(
